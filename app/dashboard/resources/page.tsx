...
export default function ResourcesPage() {
  return (
    <div className="p-6 lg:p-8 max-w-3xl">
      <h1 className="font-heading text-4xl font-semibold text-forest mb-2">Resources</h1>
      <p className="text-charcoal/55 font-body text-sm mb-8">Course materials, charts and reference documents</p>
        ...
        {resources.map((r) => (
          <div key={r.name} className="bg-white border border-forest/10 p-4 rounded-sm flex items-center gap-4 hover:border-gold/30 transition-colors">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="font-body text-xs text-charcoal/40">{r.type} · {r.size}</p>
            </div>
            <a href={r.path} download
              className="flex items-center gap-1.5 border border-forest/20 text-forest hover:bg-forest hover:text-ivory hover:border-forest px-3 py-1.5 rounded text-xs font-body font-medium transition-all flex-shrink-0">
