  ...
  return (
    <div className="p-6 lg:p-8 max-w-3xl">
      <div className="flex items-center justify-between mb-2">
        <h1 className="font-heading text-4xl font-semibold text-forest">Samyut Yoga Forum</h1>
        <button onClick={() => setNewPost(!newPost)}
          className="flex items-center gap-2 bg-forest text-ivory px-4 py-2 rounded text-sm font-body font-medium hover:bg-forest-light transition-colors">
          <Plus className="w-4 h-4" /> New Post
        </button>
      </div>
      <p className="text-charcoal/55 font-body text-sm mb-6">Community discussion for students and graduates</p>
      ...
      <div className="bg-gold/10 border border-gold/30 p-3 rounded-sm flex items-center gap-2 mb-6">
        ...
        <p className="text-charcoal/70 font-body text-xs">
      ...
      {newPost && (
        <div className="bg-white border border-forest/15 p-5 rounded-sm mb-6">
          <h3 className="font-heading text-lg font-semibold text-forest mb-3">Create New Post</h3>
          <input className="w-full border border-forest/20 rounded px-3 py-2 text-sm font-body mb-3 focus:outline-none focus:border-forest"
            placeholder="Post title..." />
          <textarea value={postText} onChange={(e) => setPostText(e.target.value)} rows={4}
            className="w-full border border-forest/20 rounded px-3 py-2 text-sm font-body mb-3 resize-none focus:outline-none focus:border-forest"
            placeholder="Share your question, insight or reflection..." />
          <div className="flex gap-3">
            <button onClick={() => { setNewPost(false); setPostText(""); }}
              className="border border-forest/20 text-forest px-4 py-2 rounded text-xs font-body hover:bg-forest/5 transition-colors">
              Cancel
            </button>
            <button className="bg-forest text-ivory px-5 py-2 rounded text-xs font-body font-medium hover:bg-forest-light transition-colors">
              Post
            </button>
          </div>
        </div>
        ...
        {threads.map((thread) => (
          <div key={thread.id} className="bg-white border border-forest/10 p-5 rounded-sm hover:border-gold/30 transition-colors cursor-pointer">
            <div className="flex items-start gap-3 mb-3">
              <span className="text-xs font-body px-2 py-0.5 bg-forest/10 text-forest rounded-sm flex-shrink-0">{thread.module}</span>
              {thread.hasTeacherReply && (
                <span className="text-xs font-body px-2 py-0.5 bg-gold/20 text-amber rounded-sm flex-shrink-0">Teacher replied</span>
              )}
            </div>
            <h3 className="font-heading text-xl font-semibold text-forest mb-2 hover:text-gold transition-colors">
              {thread.title}
            </h3>
            <p className="text-charcoal/60 font-body text-sm leading-relaxed mb-4 line-clamp-2">{thread.preview}</p>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="font-body text-xs text-charcoal/60">{thread.author}</span>
                {thread.isVIP && <Crown className="w-3.5 h-3.5 text-gold" />}
                <span className="text-charcoal/30 text-xs">·</span>
                <span className="font-body text-xs text-charcoal/40">{thread.time}</span>
              </div>
