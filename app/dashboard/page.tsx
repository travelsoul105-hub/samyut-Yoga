...
export default async function DashboardPage() {
  ...
  return (
    <div className="p-6 lg:p-8 max-w-5xl">
      {/* Greeting */}
      <div className="mb-8">
        <h1 className="font-cormorant text-4xl font-semibold text-forest mb-1">
          Namaste, {firstName} 🙏
        </h1>
        {courseName ? (
          <p className="text-charcoal/55 font-inter text-sm">{courseName}</p>
        ) : (
          <p className="text-charcoal/40 font-inter text-sm italic">No active enrolment yet</p>
        )}
      </div>
        ...
        <div className="bg-white border border-forest/10 p-6 rounded-sm flex flex-col items-center justify-center gap-3">
          <div className="relative">
            <svg width="100" height="100" viewBox="0 0 100 100" className="rotate-[-90deg]">
              ...
              <circle
                cx="50" cy="50" r="40" fill="none" stroke="#1B3A2D" strokeWidth="8"
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <p className="font-cormorant text-2xl font-bold text-forest">{progressPct}%</p>
            </div>
          </div>
          <div className="text-center">
            <p className="font-inter text-sm font-medium text-charcoal/70">Course Progress</p>
          ...
          {quickLinks.map(({ icon: Icon, label, href, color }) => (
            <Link key={label} href={href}
              ...
              <div className="flex-1 min-w-0">
                <p className="font-inter text-sm font-semibold text-forest">{label}</p>
              </div>
      ...
      {courseName && (
        <div className="bg-forest text-ivory p-5 rounded-sm flex items-center gap-4">
          ...
          <div className="flex-1">
            {progressPct === 100 ? (
              <>
                ...
                <p className="text-ivory/60 font-inter text-xs mt-0.5">
          ...
          <Link
            ...
            className="flex-shrink-0 bg-gold hover:bg-gold/90 text-forest px-4 py-2 rounded text-xs font-inter font-semibold transition-colors ml-auto"
          >
